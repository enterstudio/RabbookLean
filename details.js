...
$( document ).ready(function(){
    chrome.runtime.getBackgroundPage(function(pg){
        ...
        chrome.storage.local.get({'clist':[],  'flist':[], 'tlist':[], 'nlist':[], 'css':null, 'js':null}, function (result) {
...
//            root.bklist = tmplist;
    ...
    $('.save').click(function(){
        ...
        if(sok){
            chrome.storage.local.set({"clist":root.clist,"flist":root.flist, "nlist":root.nlist,"tlist":root.tlist},function(){
                alert("设置完成");
                window.location.reload(); 
            });
        } else {
    ...
    //Load the config
    chrome.storage.local.get({"clist":[], "flist":[], "nlist":[], "tlist":[]},function(r){
        root.clist = r.clist;
        root.tlist = r.tlist;
        root.nlist = r.nlist;
        root.flist = r.flist;
